...
function editRecipes() {
    ...
    if (mcBlocks.length == 0) {
        fetch("./src/blocks.json").then(e => e.json()).then(e => {
            mcBlocks = e;
            for (let i in e)
                mcBlocksPicker.append(`<div class="col-md-1 m-0 p-1"><img src="${e[i].texture}"></div>`);
        })
    } else {
        for (let i in mcBlocks)
            mcBlocksPicker.append(`<div class="col-md-1 m-0 p-1"><img src="${mcBlocks[i].texture}"></div>`);
    }
    ...
    $("#block-search").on('input', (e) => {
        ...
        for (let i in temp)
            mcBlocksPicker.append(`<div class="col-md-1 m-0 p-1"><img src="${temp[i].texture}"></div>`);
    });
}